...
`pdflatex -interaction=nonstopmode cv/cv.tex` — rebuild the CV. It needs `fontawesome5` (including its `.fd` files), `titlesec`, `enumitem`, and the `cm-super` `sfrm*` Type 1 fonts; without cm-super the bullets, `€` and `$` silently become a Type 3 bitmap. BasicTeX ships none of these and `tlmgr` cannot install them, so fetch them from CTAN into `~/Library/texmf` and register the maps with `updmap-user`. Copy the new PDF into `images/` under a dated name and repoint the link in `index.html`. The layout runs to exactly two pages — after adding entries, confirm the page count and check the log for `Overfull \vbox`.
